...
class Component implements Component_Interface {
	...
	protected function get_js_files() : array {
		...
		$js_files = array(
			'Lazar-theme-global-js'  => array(
				'file'         => 'global.min.js',
				'dependencies' => array( 'jquery', 'Lazar-theme-popover-js' ),
				'in_footer'    => true,
			),
			'Lazar-theme-popover-js' => array(
		 ...
		 */
		$js_files = apply_filters( 'Lazar_theme_js_files', $js_files );
	...
	public function register_global() {
		...
		foreach ( $js_files as $handle => $data ) {
			...
			$version = Lazar_theme()->get_asset_version( $js_dir . $data['file'] );
			$asset   = $js_uri . Lazar_theme()->get_asset_path( $data['file'] );
