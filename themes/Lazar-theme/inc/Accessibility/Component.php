...
class Component implements Component_Interface {
	...
	public function action_enqueue_navigation_script() {
		...
		wp_enqueue_script(
			'Lazar-theme-navigation',
			get_theme_file_uri( '/assets/js/navigation.min.js' ),
			array(),
			Lazar_theme()->get_asset_version( get_theme_file_path( '/assets/js/navigation.min.js' ) ),
			false
		);
		wp_script_add_data( 'Lazar-theme-navigation', 'async', true );
		wp_script_add_data( 'Lazar-theme-navigation', 'precache', true );
		wp_localize_script(
			'Lazar-theme-navigation',
			'lazarThemeScreenReaderText',
			array(
				'expand'   => __( 'Expand child menu', 'Lazar-theme' ),
				'collapse' => __( 'Collapse child menu', 'Lazar-theme' ),
			)
		);
	}
