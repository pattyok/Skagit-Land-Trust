...
class Component implements Component_Interface {
	...
	protected function get_js_files() : array {
		...
		$js_files = array(
			'skagit-land-trust-global-js'  => array(
				'file'         => 'global.min.js',
				'dependencies' => array( 'jquery', 'skagit-land-trust-popover-js' ),
				'in_footer'    => true,
			),
			'skagit-land-trust-popover-js' => array(
		 ...
		 */
		$js_files       = apply_filters( 'skagit_land_trust_js_files', $js_files );
		$this->js_files = array();
	...
	public function register_global() {
		...
		foreach ( $js_files as $handle => $data ) {
			...
			$version = skagit_land_trust()->get_asset_version( $js_dir . $data['file'] );
			$asset   = $js_uri . skagit_land_trust()->get_asset_path( $data['file'] );
