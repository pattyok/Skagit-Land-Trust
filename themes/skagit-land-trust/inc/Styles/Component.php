...
use function WP_Rig\WP_Rig\skagit_land_trust;
 ...
 * * `skagit_land_trust()->print_styles()`
 */
class Component implements Component_Interface, Templating_Component_Interface {
	...
	public function action_enqueue_styles() {
		...
		if ( ! empty( $google_fonts_url ) ) {
			wp_enqueue_style( 'skagit-land-trust-fonts', $google_fonts_url, array(), null ); // phpcs:ignore WordPress.WP.EnqueuedResourceParameters.MissingVersion
		}

		// Enqueue Typekit fonts if using.
		$typekit_fonts_url = $this->get_typekit_fonts_url();
		if ( ! empty( $typekit_fonts_url ) ) {
			wp_enqueue_style( 'skagit-land-trust-typekit', $typekit_fonts_url, array(), null ); // phpcs:ignore WordPress.WP.EnqueuedResourceParameters.MissingVersion
		}
		...
		if ( ! empty( $fontawesome_kit_url ) ) {
			...
			wp_enqueue_script( 'skagit-land-trust-fa', $fontawesome_kit_url, array(), null, array( 'in_footer' => true ) ); // phpcs:ignore WordPress.WP.EnqueuedResourceParameters.MissingVersion
		}
		...
		foreach ( $css_files as $handle => $data ) {
			$version = skagit_land_trust()->get_asset_version( $css_dir . $data['file'] );
			$asset   = $css_uri . skagit_land_trust()->get_asset_path( $data['file'] );
	...
	public function action_add_editor_styles() {
		...
		$google_fonts_url = $this->get_google_fonts_url();
		if ( ! empty( $google_fonts_url ) ) {
			wp_enqueue_style( 'skagit-land-trust-google-fonts', $google_fonts_url, array(), null ); // phpcs:ignore WordPress.WP.EnqueuedResourceParameters.MissingVersion
		}
	...
	public function action_add_block_styles() {
		...
		foreach ( $styled_blocks as $prefix => $blocks ) {
			foreach ( $blocks as $block_name ) {
				$args = array(
					'handle' => "trc-fse-$block_name",
					'src'    => get_theme_file_uri( "assets/css/blocks/$block_name.min.css" ),
					'ver'    => filemtime( get_theme_file_path( "assets/css/blocks/$block_name.min.css" ) ),
				);
				// Replace the "core" prefix if you are styling blocks from plugins.
				wp_enqueue_block_style( "$prefix/$block_name", $args );
	...
	public function filter_resource_hints( array $urls, string $relation_type ): array {
		if ( 'preconnect' === $relation_type && wp_style_is( 'skagit-land-trust-fonts', 'queue' ) ) {
			$urls[] = array(
		...
		$handles   = array_filter(
			...
			function ( $handle ) use ( $css_files ) {
				...
				if ( ! $is_valid ) {
					...
					_doing_it_wrong( __CLASS__ . '::print_styles()', esc_html( sprintf( __( 'Invalid theme stylesheet handle: %s', 'skagit-land-trust' ), $handle ) ), 'Skagit Land Trust - Custom Theme 2.0.0' );
				}
				return $is_valid;
			}
		);
	 ...
	 * {@see 'skagit_land_trust_preloading_styles_enabled'} filter can be used to tweak the return value.
	 *
	...
	protected function preloading_styles_enabled() {
		 ...
		 */
		return apply_filters( 'skagit_land_trust_preloading_styles_enabled', $preloading_styles_enabled );
	}
	...
	protected function get_css_files(): array {
		...
		$css_files = array(
			'skagit-land-trust-global' => array(
				'file'   => 'global.min.css',
				'global' => true,
			),
		);
		 ...
		 */
		$css_files = apply_filters( 'skagit_land_trust_css_files', $css_files );
	...
	protected function get_google_fonts(): array {
		 ...
		 */
		$this->google_fonts = (array) apply_filters( 'skagit_land_trust_google_fonts', $google_fonts );
	...
	/**
	 * Returns the Fontawesome URL to use for enqueuing.
	 *
	 * If not using just empty the string.
	 *
	 * @return string  URL, or empty string if no kit should be used.
	 */
	protected function get_fontawesome_fonts_url(): string {
		return '';
	}
}
