...
function initEachNavToggleSubmenu( nav ) {
	...
	for ( let i = 0; i < SUBMENUS.length; i++ ) {
		...
		const parentLink = parentMenuItem.firstElementChild.cloneNode( true );
		...
		// Clone the parent element link and button to create the "back" button on Mobile
		const backItem = document.createElement( 'li' );
		backItem.classList.add( 'mobile-menu-item-back' );

		const backItemLabel = document.createTextNode( 'Back' );
		backItem.appendChild( backItemLabel );
		SUBMENUS[ i ].insertBefore( backItem, firstSubItem );

		backItem.appendChild( childDropdownButton );

		//add parent link as first item in submenu for mobile;
		const parentItem = document.createElement( 'li' );
		parentItem.classList.add( 'mobile-menu-item-parent' );
		parentItem.appendChild( parentLink );
		SUBMENUS[ i ].insertBefore( parentItem, firstSubItem );
