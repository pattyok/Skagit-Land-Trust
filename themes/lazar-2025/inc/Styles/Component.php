...
class Component implements Component_Interface, Templating_Component_Interface {
	...
	/**
	 * Gets template tags to expose as methods on the Template_Tags class instance, accessible through `lazar_2025()`.
	 *
	...
	public function action_enqueue_styles() {
		...
		if ( ! empty( $google_fonts_url ) ) {
			wp_enqueue_style( 'lazar-2025-fonts', $google_fonts_url, array(), null ); // phpcs:ignore WordPress.WP.EnqueuedResourceParameters.MissingVersion
		}

		// Enqueue Typekit fonts if using.
		$typekit_fonts_url = $this->get_typekit_fonts_url();
		if ( ! empty( $typekit_fonts_url ) ) {
			wp_enqueue_style( 'lazar-2025-typekit', $typekit_fonts_url, array(), null ); // phpcs:ignore WordPress.WP.EnqueuedResourceParameters.MissingVersion
		}
		...
		if ( ! empty( $fontawesome_kit_url ) ) {
			...
			wp_enqueue_script( 'lazar-2025-fa', $fontawesome_kit_url, array(), null, array( 'in_footer' => true ) ); // phpcs:ignore WordPress.WP.EnqueuedResourceParameters.MissingVersion
		}
		...
		foreach ( $css_files as $handle => $data ) {
			$version = lazar_2025()->get_asset_version( $css_dir . $data['file'] );
			$asset   = $css_uri . lazar_2025()->get_asset_path( $data['file'] );
	...
	public function action_add_editor_styles() {
		...
		$typekit_fonts_url = $this->get_typekit_fonts_url();
		if ( ! empty( $typekit_fonts_url ) ) {
			add_editor_style( 'lazar-2025-typekit', $typekit_fonts_url, array(), null ); // phpcs:ignore WordPress.WP.EnqueuedResourceParameters.MissingVersion
		}
	...
	public function filter_resource_hints( array $urls, string $relation_type ): array {
		if ( 'preconnect' === $relation_type && wp_style_is( 'lazar-2025-fonts', 'queue' ) ) {
			$urls[] = array(
		...
		$handles   = array_filter(
			...
			function ( $handle ) use ( $css_files ) {
				...
				if ( ! $is_valid ) {
					...
					_doing_it_wrong( __CLASS__ . '::print_styles()', esc_html( sprintf( __( 'Invalid theme stylesheet handle: %s', 'lazar-2025' ), $handle ) ), 'Lazar Landscape 2025 2.0.0' );
				}
				return $is_valid;
			}
		);
	 ...
	 * {@see 'lazar_2025_preloading_styles_enabled'} filter can be used to tweak the return value.
	 *
	...
	protected function preloading_styles_enabled() {
		 ...
		 */
		return apply_filters( 'lazar_2025_preloading_styles_enabled', $preloading_styles_enabled );
	}
	...
	protected function get_css_files(): array {
		 ...
		 */
		$css_files = apply_filters( 'lazar_2025_css_files', $css_files );
	...
	protected function get_google_fonts(): array {
		 ...
		 */
		$this->google_fonts = (array) apply_filters( 'lazar_2025_google_fonts', $google_fonts );
